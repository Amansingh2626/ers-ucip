...
public class UCIPAdaptor31 extends IUCIPAdaptor
{
	...
	Logger logger = LoggerFactory.getLogger(UCIPAdaptor31.class);

	public UCIPAdaptor31(UCIPLinkConfig config, ExtendedProperties properties)
	{
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("UCIPAdaptor31::addToAccountFAF");
		...
		try
		{
			...
			com.seamless.common.uciplib.v31.requests.UpdateFaFListRequest request = new com.seamless.common.uciplib.v31.requests.UpdateFaFListRequest(subscriberNumber, "ADD", fafInformation);
			resetExternalDataParameters();
			setDefaultVariables(request);
			request.setOriginTransactionID(transactionId);
			com.seamless.common.uciplib.v31.responses.UpdateFaFListResponse response = (UpdateFaFListResponse) makeRequest(request);
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("UCIPAdaptor31::removeAccountFAF");
		...
		try
		{
			...
			fafInformation[0].setFafIndicator(config.getFafIndicator());
			fafInformation[0].setOwner("Subscriber");
			com.seamless.common.uciplib.v31.requests.UpdateFaFListRequest request = new com.seamless.common.uciplib.v31.requests.UpdateFaFListRequest(subscriberNumber, "DELETE", fafInformation);
			resetExternalDataParameters();
			setDefaultVariables(request);
			request.setOriginTransactionID(transactionId);
			com.seamless.common.uciplib.v31.responses.UpdateFaFListResponse response = (UpdateFaFListResponse) makeRequest(request);
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("UCIPAdaptor31::getAccountDetails");
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("UCIPAdaptor31::getAccountFAF");
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("UCIPAdaptor31::getBalance");
	 ...
	 * {@inheritDoc}
	 */
	public UCIPResponse refill(String subscriberNumber, Amount amount, String transactionId, String transactionType,
			String profileId, Integer dedicatedAccountId, ERSHashtableParameter extraFields)
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("UCIPAdaptor31::refill");
		...
		try
		{
			...
			result.setUcipResponseCode(response.getResponseCode());
			handleAccountDetails(dedicatedAccountId, response, result);
		}
		catch (ConnectException e)
		{
			String err = "The connection to the CS is down: " + e.getMessage();
	...
	private void handleAccountDetails(Integer dedicatedAccountId, RefillResponse response,
			UCIPResponse result) throws ERSInvalidCurrencyException
	{
	 ...
	 * {@inheritDoc}
	 */
	public UCIPResponse updateServiceClass(String subscribernumber, int serviceClass, String transactionId,
			ERSHashtableParameter extraFields)
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("UCIPAdaptor31::updateServiceClass");
	...
	public UCIPResponse updateBalance(String subscriberNumber, Amount relativeBalance, String transactionId,
			String transactionType, Integer dedicatedAccountId, ERSHashtableParameter extraFields)
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("UCIPAdaptor31::updateBalance");
	...
	public UCIPResponse updateAccumulators(String subscriberNumber, String transactionId, AccumulatorUpdateInformation[] info,ERSHashtableParameter extraFields)
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint(
				"UCIPAdaptor31::updateAccumulators");
		point.collect();
	...
	private Amount getDedicatedAccountBalance(DedicatedAccountInformation[] daInfoList, int dedicatedAccountId,
			String currency) throws ERSInvalidCurrencyException
	{
		if (daInfoList != null)
		{
	...
	@Override
	public UCIPResponse redeemVoucher(String subscribernumber, String voucherCode, String transactionId,
			ERSHashtableParameter extraFields)
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("UCIPAdaptor31::redeemVoucher");
		...
		try
		{
			...
			if (response.getAccountAfterRefill() != null)
			{
				// FIXME account value should be divided by currency decimal
				// digits
	...
	/**
	 * Added for Data Bundle request but currently not supported in v31
	 *
	 ...
	 * @return
	 */
	public UCIPResponse updateOffer(String subscriberNumber, Amount amount, String transactionId,
			String transactionType, String profileId, Integer dedicatedAccountId, ERSHashtableParameter extraFields,
